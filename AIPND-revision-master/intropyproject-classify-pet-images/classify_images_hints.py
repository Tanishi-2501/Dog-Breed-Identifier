...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
        ...
        # Get the model label from the classifier function
        model_label = classifier(image_path, model)

        # Process the model label to lowercase and strip whitespace
        model_label = model_label.lower().strip()
        
        # Get the true label
        truth = results_dic[key][0]
        
        # Determine if the true label is found in the model label
        match = 1 if truth in model_label else 0
        
        # Update the results dictionary with the classifier label and match result
